function [gbest,gbestval,fitcount]= PSO_levy(fhd,Dimension,Particle_Number,Max_Gen,VRmin,VRmax,varargin)
...
alpha = 1.5;
beta = 1/2;
gamma_pool = min(0.1*ones(1,D), (VRmax(1,:)-VRmin(1,:))/1000);   % scale
theta = 0;               % position
...
for i=2:me
        ...
        for kk = 1:D
        gamma = gamma_pool(kk);
        vel(:, kk) = stblrnd(alpha, beta, gamma, theta, [1 ps]).*inertia.*vel(:,kk)' ...
            + corr_factor.*stblrnd(alpha, beta, gamma, theta, [1 ps]).*(pbest(:,kk) - pos(:, kk))' ...
            + corr_factor.*stblrnd(alpha, beta, gamma, theta, [1 ps]).*(gbest(kk) - pos(:, kk))';   %x velocity component
        end
